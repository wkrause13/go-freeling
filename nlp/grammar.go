...
func NewGrammar(fname string) *Grammar {
	...
	rules = append(rules, &Pair{regexp.MustCompile("\\([\\p{L}_'·\\-]+\\)"), GRAMMAR_FORM})
	rules = append(rules, &Pair{regexp.MustCompile("<[\\p{Ll}_'·\\-]+>"), GRAMMAR_LEMMA})
	rules = append(rules, &Pair{regexp.MustCompile("\\(\\\"(\\p{L}:)?[\\p{L}\\d_\\-\\./\\\\]+\\\"\\)"), GRAMMAR_FILENAME})
	rules = append(rules, &Pair{regexp.MustCompile("<\\\"(\\p{L}:)?[\\p{L}\\d_\\-\\./\\\\]+\\\">"), GRAMMAR_FILENAME})
	rules = append(rules, &Pair{regexp.MustCompile("\\p{L}[\\-A-Za-z0-9]*[*]?"), GRAMMAR_CATEGORY})
